...
import {ProgramStateViewer} from "../src/scenes/Scene.js";
...
export class Body {
    ...
    static intersectCube(p, margin = 0) {
        return p.every(value => value >= -1 - margin && value <= 1 + margin);
    }

    static intersectSphere(p, margin = 0) {
        return p.dot(p) < 1 + margin;
    }

    emplace(locationMatrix, linearVelocity, angularVelocity, spinAxis = vec3(0, 0, 0).randomized(1).normalized()) {                               // emplace(): assign the body's initial values, or overwrite them.
        this.center = locationMatrix.times(vec4(0, 0, 0, 1)).to3();
        this.rotation = Mat4.translation(...this.center.times(-1)).times(locationMatrix);
        this.previous = {center: this.center.copy(), rotation: this.rotation.copy()};
        // drawn_location gets replaced with an interpolated quantity:
        this.drawn_location = locationMatrix;
        this.temp_matrix = Mat4.identity();
        return Object.assign(this, {linearVelocity, angularVelocity, spinAxis});
    }
    ...
    advance(timeAmount) {
        ...
        this.previous = {center: this.center.copy(), rotation: this.rotation.copy()};
        // Apply the velocities scaled proportionally to real time (timeAmount):
        // Linear velocity first, then angular:
        this.center = this.center.plus(this.linearVelocity.times(timeAmount));
        this.rotation.pre_multiply(Mat4.rotation(timeAmount * this.angularVelocity, ...this.spinAxis));
    }
    ...
    blendRotation(alpha) {
        // blendRotation(): Just naively do a linear blend of the rotations, which looks
    ...
    blendState(alpha) {
        // blendState(): Compute the final matrix we'll draw using the previous two physical
        // locations the object occupied.  We'll interpolate between these two states as
        // described at the end of the "Fix Your Timestep!" blog post.
        this.drawn_location = Mat4.translation(...this.previous.center.mix(this.center, alpha))
            .times(this.blendRotation(alpha))
            .times(Mat4.scale(...this.size));
    }

    checkIfColliding(b, collider) {
        // checkIfColliding(): Collision detection function.
        ...
        const {intersectTest, points, leeway} = collider;
        // For each vertex in that b, shift to the coordinate frame of
        // a_inv*b.  Check if in that coordinate frame it penetrates
        // the unit sphere at the origin.  Leave some leeway.
        return points.arrays.position.some(p =>
            intersectTest(t.times(p.to4(1)).to3(), leeway));
    }
}
...
export class Simulation extends Scene {
    ...
    simulate(frameTime) {
        ...
        frameTime = this.time_scale * frameTime;

        // Avoid the spiral of death; limit the amount of time we will spend
        // computing during this timestep if display lags:
        this.time_accumulator += Math.min(frameTime, 0.1);
        ...
        while (Math.abs(this.time_accumulator) >= this.dt) {
            ...
            this.t += Math.sign(frameTime) * this.dt;
            this.time_accumulator -= Math.sign(frameTime) * this.dt;
            this.steps_taken++;
        }
        ...
        for (let b of this.bodies) b.blendState(alpha);
    }

    makeControlPanel() {
        // makeControlPanel(): Create the buttons for interacting with simulation time.
        this.keyTriggeredButton("Speed up time", ["Shift", "T"], () => this.time_scale *= 5);
        this.keyTriggeredButton("Slow down time", ["t"], () => this.time_scale /= 5);
        this.newLine();
        this.liveString(box => {
    ...
    display(context, programState) {
        ...
        if (programState.animate)
            this.simulate(programState.animationDeltaTime);
        // Draw each shape at its current location:
        for (let b of this.bodies)
            b.shape.draw(context, programState, b.drawn_location, b.material);
    }

    updateState(dt)      // updateState(): Your subclass of Simulation has to override this abstract function.
    {
        throw "Override this";
    }
}
...
export class TestData {
    ...
    randomShape(shapeList = this.shapes) {
        // randomShape():  Extract a random shape from this.shapes.
        const shapeNames = Object.keys(shapeList);
        return shapeList[shapeNames[~~(shapeNames.length * Math.random())]];
    }
}


export class InertiaDemo extends Simulation {
    // ** InertiaDemo** demonstration: This scene lets random initial momentums
    ...
    updateState(dt) {
        // updateState():  Override the base time-stepping code to say what this particular
        // scene should do to its bodies every frame -- including applying forces.
        // Generate additional moving bodies if there ever aren't enough:
        while (this.bodies.length < 150)
            this.bodies.push(new Body(this.data.randomShape(), this.randomColor(), vec3(1, 1 + Math.random(), 1))
                .emplace(Mat4.translation(...vec3(0, 15, 0).randomized(10)),
                    vec3(0, -1, 0).randomized(2).normalized().times(3), Math.random()));

        for (let b of this.bodies) {
            // Gravity on Earth, where 1 unit in world space = 1 meter:
            b.linearVelocity[1] += dt * -9.8;
            // If about to fall through floor, reverse y velocity:
            if (b.center[1] < -8 && b.linearVelocity[1] < 0)
                b.linearVelocity[1] *= -.8;
        }
        // Delete bodies that stop or stray too far away:
        this.bodies = this.bodies.filter(b => b.center.norm() < 50 && b.linearVelocity.norm() > 2);
    }

    display(context, programState) {
        // display(): Draw everything else in the scene besides the moving bodies.
        super.display(context, programState);

        if (!context.scratchpad.controls) {
            this.children.push(context.scratchpad.controls = new MovementControls());
            this.children.push(new ProgramStateViewer());
            programState.setCamera(Mat4.translation(0, 0, -50));    // Locate the camera here (inverted matrix).
        }
        programState.projectionTransform = Mat4.perspective(Math.PI / 4, context.width / context.height, 1, 500);
        programState.lights = [new Light(vec4(0, -5, -10, 1), color(1, 1, 1, 1), 100000)];
        // Draw the ground:
        this.shapes.square.draw(context, programState, Mat4.translation(0, -10, 0)
...
export class CollisionDemo extends Simulation {
    // **CollisionDemo** demonstration: Detect when some flying objects
    ...
    constructor() {
        ...
        this.colliders = [
            {intersectTest: Body.intersectSphere, points: new SubdivisionSphere(1), leeway: .5},
            {intersectTest: Body.intersectSphere, points: new SubdivisionSphere(2), leeway: .3},
            {intersectTest: Body.intersectCube, points: new Cube(), leeway: .1},
        ];
    ...
    makeControlPanel() {
        this.keyTriggeredButton("Previous collider", ["b"], this.decrease);
        this.keyTriggeredButton("Next", ["n"], this.increase);
        this.newLine();
        super.makeControlPanel();
    }
    ...
    updateState(dt, numBodies = 40) {
        // updateState():  Override the base time-stepping code to say what this particular
        // scene should do to its bodies every frame -- including applying forces.
        // Generate moving bodies:
        while (this.bodies.length < numBodies)
            this.bodies.push(new Body(this.data.randomShape(), undefined, vec3(1, 5, 1))
        ...
        this.bodies = this.bodies.filter(b => (Math.random() > .01) || b.linearVelocity.norm() > 1);
        ...
        for (let a of this.bodies) {
            ...
            a.linearVelocity = a.linearVelocity.minus(a.center.times(dt));
            // Apply a small centripetal force to everything.
            a.material = this.inactive_color;
            // Default color: white

            if (a.linearVelocity.norm() == 0)
                continue;
            // *** Collision process is here ***
            // Loop through all bodies again (call each "b"):
            for (let b of this.bodies) {
                // Pass the two bodies and the collision shape to checkIfColliding():
                if (!a.checkIfColliding(b, collider))
                    continue;
    ...
    display(context, programState) {
        ...
        if (!context.scratchpad.controls) {
            this.children.push(context.scratchpad.controls = new MovementControls());
            this.children.push(new ProgramStateViewer());
            programState.setCamera(Mat4.translation(0, 0, -50));
            // Locate the camera here (inverted matrix).
        }
        programState.projectionTransform = Mat4.perspective(Math.PI / 4, context.width / context.height, 1, 500);
        programState.lights = [new Light(vec4(.7, 1.5, 2, 0), color(1, 1, 1, 1), 100000)];
        ...
        for (let b of this.bodies)
            points.draw(context, programState, b.drawn_location.times(Mat4.scale(...size)), this.bright, "LINE_STRIP");
    }

    showExplanation(documentElement) {
        documentElement.innerHTML += `<p>This demo detects when some flying objects collide with one another, coloring them red when they do.  For a simpler demo that shows physics-based movement without objects that hit one another, see the demo called Inertia_Demo.
