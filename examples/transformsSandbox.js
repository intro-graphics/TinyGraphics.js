...
export class TransformsSandboxBase extends Scene {
    ...
    makeControlPanel() {
        ...
        this.controlPanel.innerHTML += "Dragonfly rotation angle: ";
        ...
        this.keyTriggeredButton("Hover dragonfly in place", ["h"], function () {
            this.hover ^= 1;
        });
        this.newLine();
    }
    ...
    display(context, programState) {
        ...
        if (!context.scratchpad.controls) {
            ...
            // Define the global camera and projection matrices, which are stored in programState.  The camera
            // matrix follows the usual format for transforms, but with opposite values (cameras exist as
            // inverted matrices).  The projection matrix follows an unusual format and determines how depth is
            // treated when projecting 3D points onto a plane.  The Mat4 functions perspective() and
            // orthographic() automatically generate valid matrices for one.  The input arguments of
            // perspective() are field of view, aspect ratio, and distances to the near plane and far plane.
            programState.setCamera(Mat4.translation(0, 3, -10));
        }
        programState.projectionTransform = Mat4.perspective(
        ...
        const lightPosition = Mat4.rotation(angle, 1, 0, 0).times(vec4(0, -1, 1, 0));
        programState.lights = [new Light(lightPosition, color(1, 1, 1, 1), 1000)];
    }
}
...
class TransformsSandbox extends TransformsSandboxBase {
    ...
    display(context, programState) {
        ...
        // programState:  Information the shader needs for drawing.  Pass to draw().
        ...
        modelTransform = modelTransform.times(Mat4.translation(0, 0, 0));
        // Draw the top box:
        this.shapes.box.draw(context, programState, modelTransform, this.materials.plastic.override(yellow));
        ...
        this.shapes.ball.draw(context, programState, modelTransform, this.materials.metal.override(blue));
        ...
        if (!this.hover)
            modelTransform = modelTransform.times(Mat4.rotation(t, 0, 1, 0));
        ...
        modelTransform = modelTransform.times(Mat4.rotation(1, 0, 0, 1))
            .times(Mat4.scale(1, 2, 1))
            .times(Mat4.translation(0, -1.5, 0));
        // Draw the bottom (child) box:
        this.shapes.box.draw(context, programState, modelTransform, this.materials.plastic.override(yellow));

        // Note that our coordinate system stored in modelTransform still has non-uniform scaling
        ...
        // the scaled matrix back in modelTransform at all -- but instead in just a temporary
