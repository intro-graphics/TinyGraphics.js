...
export class ShapeFromFile extends Shape {                                   // **ShapeFromFile** is a versatile standalone Shape that imports
    // all its arrays' data from an .obj 3D model file.
    constructor(filename) {
        super("position", "normal", "texture_coord");
        // Begin downloading the mesh. Once that completes, return
        // control to our parseIntoMesh function.
        this.loadFile(filename);
    }

    loadFile(filename) {                             // Request the external file and wait for it to load.
        ...
        return fetch(filename)
            ...
            .then(obj_file_contents => this.parseIntoMesh(obj_file_contents))
            .catch(error => {
            ...
            if (VERTEX_RE.test(line)) verts.push.apply(verts, elements);
                ...
                for (var j = 0, eleLen = elements.length; j < eleLen; j++) {
                    ...
                    if (elements[j] in unpacked.hashindices)
                        ...
                        unpacked.verts.push(+verts[(vertex[0] - 1) * 3]);
                        unpacked.verts.push(+verts[(vertex[0] - 1) * 3 + 1]);
                        unpacked.verts.push(+verts[(vertex[0] - 1) * 3 + 2]);

                        if (textures.length) {
                            unpacked.textures.push(+textures[((vertex[1] - 1) || vertex[0]) * 2]);
                            unpacked.textures.push(+textures[((vertex[1] - 1) || vertex[0]) * 2 + 1]);
                        }

                        unpacked.norms.push(+vertNormals[((vertex[2] - 1) || vertex[0]) * 3]);
    ...
    draw(context, programState, model_transform, material) {               // draw(): Same as always for shapes, but cancel all
        // attempts to draw the shape before it loads:
        if (this.ready)
            super.draw(context, programState, model_transform, material);
    }
}


/**
 * **ObjFileDemo** show how to load a single 3D model from an OBJ file.
...
export class ObjFileDemo extends Scene {
    ...
    display(context, programState) {
        ...
        programState.setCamera(Mat4.translation(0, 0, -5));    // Locate the camera here (inverted matrix).
